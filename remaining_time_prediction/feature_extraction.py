...
def build_remaining_time_dataset(
    log,
    ...
    for trace in log:
        ...
        end_time = events[-1].get(timestamp_key)
        if end_time is None:
            continue
        ...
        for idx in range(1, len(events)):
            ...
            if curr_ts is None:
                continue
            remaining_seconds = (end_time - curr_ts).total_seconds()
            features.append(row)
            targets.append(float(remaining_seconds))
