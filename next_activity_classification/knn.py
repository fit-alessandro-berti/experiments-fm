...
def evaluate_log(
    ...
    if len(x_train) < n_neighbors:
        result["status"] = "skipped"
        result["reason"] = (
            f"Training data has fewer rows ({len(x_train)}) than k ({n_neighbors})."
        )
        return result
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Train/evaluate a kNN next-activity classifier using "
    ...
    parser.add_argument("--method-name", default="knn")
    parser.add_argument(
        "--k",
        "--n-neighbors",
        dest="n_neighbors",
    ...
    if args.n_neighbors < 1:
        raise SystemExit("--k/--n-neighbors must be >= 1")
