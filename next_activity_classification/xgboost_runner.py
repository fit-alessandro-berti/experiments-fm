...
def evaluate_log(
    ...
    x_train, y_train = build_next_activity_dataset(
        train_log,
        feature_space=feature_space,
        activity_key=activity_key,
        timestamp_key=timestamp_key,
    )
    x_test, y_test = build_next_activity_dataset(
        test_log,
        feature_space=feature_space,
        activity_key=activity_key,
        timestamp_key=timestamp_key,
    ...
    if len(set(y_train)) < 2:
        result["status"] = "skipped"
        result["reason"] = "Training data has fewer than 2 classes."
        return result

    # XGBoost expects contiguous class ids [0, ..., K-1]. Prefix extraction can
    # produce sparse ids if some activities are absent in the sampled training set.
    train_classes = sorted(set(y_train))
    class_to_encoded = {cls: idx for idx, cls in enumerate(train_classes)}
    encoded_to_class = {idx: cls for cls, idx in class_to_encoded.items()}
    y_train_encoded = np.asarray([class_to_encoded[cls] for cls in y_train], dtype=np.int64)
    ...
    y_pred = [encoded_to_class[int(encoded)] for encoded in y_pred_encoded]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Train/evaluate an XGBoost next-activity classifier using "
    ...
    parser.add_argument("--method-name", default="xgboost")
