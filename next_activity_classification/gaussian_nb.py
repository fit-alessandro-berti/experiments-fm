...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Train/evaluate a Gaussian Naive Bayes next-activity classifier using "
    ...
    parser.add_argument("--method-name", default="gaussian_nb")
    parser.add_argument(
        "--var-smoothing",
        type=float,
        default=1e-9,
        help="Portion of the largest variance added to all variances for stability.",
    )
    ...
    if args.var_smoothing < 0:
        raise SystemExit("--var-smoothing must be >= 0")
